      ...
      <div className="max-w-[1000px] mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
        ...
        {SIZES.map((entry) => (
          <div key={entry.sport} className="mb-11">
            <h2 className="font-display uppercase text-2xl mb-1.5 text-[#13251C]">{entry.sport}</h2>
            <p className="text-sm text-[#3C4536] mb-4 max-w-[60ch]">{entry.note}</p>
            <div className="border border-[#D8DED2] rounded-sm overflow-hidden bg-white">
              <div className="grid grid-cols-3 bg-[#13251C] text-white text-[11px] font-bold tracking-widest">
              ...
              {entry.rows.map((row) => (
                <div
                  key={row.size}
                  className="grid grid-cols-3 border-t border-[#D8DED2] text-[15px]"
                >
                  <div className="p-3.5 font-bold text-[#13251C]">{row.size}</div>
                  <div className="p-3.5 text-[#3C4536]">{row.age}</div>
                  <div className="p-3.5 text-[#3C4536]">{row.use}</div>
                </div>
              ))}
            </div>
          </div>
