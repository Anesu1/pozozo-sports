...
import { notFoundPageQuery, sportsQuery } from '@/sanity/lib/queries';
import { cleanSportColors } from '@/sanity/lib/stega-safe';
import { NotFoundPageContent, SportMeta } from '@/types';
...
export default async function NotFound() {
  const [{ data }, { data: contentData }] = await Promise.all([
    ...
    sanityFetch({ query: notFoundPageQuery }),
  ]);
  const SPORTS = (data as SportMeta[]).map(cleanSportColors);
  const content = contentData as NotFoundPageContent;
